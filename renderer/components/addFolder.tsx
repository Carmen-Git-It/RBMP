...
export default function AddFolder() {
  ...
  // data of format {path1: [{filePath, duration}, ...], path2: [{filePath, duration}, ...] ...}
            ...
            <React.Fragment>
              ...
              {Object.keys(folderData).map((index, key) => (
                <Paper key={key} elevation={3} sx={{ marginTop: 1 }}>
                  ...
                  <Button
                    onClick={() => {
                      changeListStatus(key);
                    }}
                  >
                    {listExpanded[key] === undefined || listExpanded[key]
                      ? "Hide Files"
                      : "Show Files"}
                  </Button>
                  <List sx={{ paddingTop: 1, paddingBottom: 2 }}>
                    {(listExpanded[key] === undefined || listExpanded[key]) &&
                      folderData[index].map((file, fileKey: number) => (
                        <ListItem key={fileKey + 20000}>
                          <ListItemText primary={file.filePath} />
                          <ListItemText
                            primary={"Duration: " + file.duration + "s"}
                          ></ListItemText>
                        </ListItem>
                      ))}
                  </List>
