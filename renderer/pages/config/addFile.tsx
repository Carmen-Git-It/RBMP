import { Box, Button, FormControl, InputLabel, MenuItem, Modal, Select, SelectChangeEvent, Stack, TextField, Typography } from "@mui/material";
        ...
        selectMediaFile().then((data) => {
            if (data) {
                if (data.filePath.includes('/')) {
                    setFileName(data.filePath.split('/')[data.filePath.split('/').length - 1]);
                } else if (data.filePath.includes('\\')) {
                    setFileName(data["fileName"] = data.filePath.split('\\')[data.filePath.split('\\').length - 1]);
                }
                setFileData(data);
            }
        });
        ...
        <React.Fragment>
            <Modal
                ...
                <Box sx={{position: 'absolute', 
                    ...
                    p: 4,}}>
                    <Stack spacing={2}>
                        <Typography id="modal-title" variant="h6" component="h2">Add Media File</Typography>
                        <TextField label="File Name" error={fileName === ""} onChange={handleFileNameChange} defaultValue={fileName}/>
                        {fileData && <Typography variant="body1">{"Duration: " + fileData.duration + "s"}</Typography>}
                        ...
                        <Stack spacing={2} direction="row" sx={{paddingTop: 2}}>
                            ...
                            <Button variant="contained" color="success" onClick={handleModalSave}>Save File</Button>
                        </Stack>
                    </Stack>
            ...
            <Modal
                ...
                <Box sx={{position: 'absolute', 
                    ...
                    <TextField id="new-type-name" label="Name" variant="outlined" defaultValue={newTypeName} onChange={handleChangeTypeName} error={newTypeName === ""} sx={{paddingTop: 2}}/>
                    <Stack spacing={2} direction="row" sx={{paddingTop: 2}}>
